...
public class UserController {
    ...
    public ServerReponse<User> getUserInfo(HttpSession session){
        ...
        return ServerReponse.createByErrorMessage("用户未登陆，无法获得当前用户的信息");
    }
    ...
    public ServerReponse<User> get_information(HttpSession session){
        ...
        User currentUser = (User)session.getAttribute(Const.CURRENT_USER);
        if(currentUser == null){
            return ServerReponse.createByErrorCodeMessage(ResponseCode.NEED_LOGIN.getCode(),"未登录，需要强制登录status=10");
        }
